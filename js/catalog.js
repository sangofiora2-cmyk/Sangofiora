...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    chips.forEach(function (c) {
      if (c.dataset.filter === currentCategory) {
        c.classList.add('bg-plant-main', 'text-white', 'shadow-sm');
        c.classList.remove('border', 'border-gray-200', 'text-gray-700');
      } else {
        c.classList.remove('bg-plant-main', 'text-white', 'shadow-sm');
        c.classList.add('border', 'border-gray-200', 'text-gray-700');
      }
    });
    ...
    chips.forEach(function (chip) {
      chip.addEventListener('click', function () {
        ...
        chip.classList.add('bg-plant-main', 'text-white', 'shadow-sm');
        chip.classList.remove('border', 'border-gray-200', 'text-gray-700');
    ...
    // Dynamic Supabase Sync (removes deleted products, updates sold-out status, adds new items)
    function syncCatalogWithSupabase() {
      ...
      window.SangoSupabase.from('products').select('*').eq('is_active', true).then(function (res) {
        ...
        // 1. Remove cards from DOM if they were deleted in Supabase or marked inactive
        cards.forEach(function (card) {
          var btn = card.querySelector('[data-add-to-cart]');
          var slug = btn ? btn.dataset.slug : null;
          if (slug && !dbSlugsMap[slug]) {
            card.remove(); // Permanently deleted by admin
        ...
        // 2. Add newly created products that aren't in the static HTML grid yet
        ...
        // 3. Update filter chip labels dynamically with real-time active product counts
        var categoryCounts = {};
        dbProducts.forEach(function (p) {
    ...
    // Initial Filter Run & Supabase Sync
    applyCatalogFilters();
    setTimeout(syncCatalogWithSupabase, 300);
  });
